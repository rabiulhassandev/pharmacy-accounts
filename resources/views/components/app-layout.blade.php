...
<body class="bg-gray-900 text-gray-100 font-sans antialiased flex min-h-screen">
    ...
    <aside class="w-64 bg-gray-800/50 backdrop-blur-md border-r border-gray-700/50 flex flex-col hidden md:flex sticky top-0 h-screen">
        ...
        <nav class="flex-1 p-4 space-y-2">
            ...
            <a href="{{ route('expenses.index') }}" class="flex items-center px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('expenses.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                Expenses
            </a>

            <a href="{{ route('suppliers.index') }}" class="flex items-center px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('suppliers.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                Suppliers
            </a>

            <a href="{{ route('customers.index') }}" class="flex items-center px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('customers.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white' }}">
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="md:hidden bg-gray-800/80 backdrop-blur-md border-b border-gray-700/50 p-4 flex items-center justify-between sticky top-0 z-20">
            ...
            <details class="relative">
                <summary class="flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-xl border border-gray-600/80 bg-gray-900/70 text-gray-200 transition-colors hover:border-emerald-400/40 hover:text-white [&::-webkit-details-marker]:hidden" aria-label="Open navigation menu">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
                    </svg>
                </summary>

                <div class="absolute right-0 top-full z-30 mt-3 w-72 overflow-hidden rounded-2xl border border-gray-700/70 bg-gray-900/95 p-3 shadow-2xl shadow-black/40 backdrop-blur-xl">
                    <nav class="space-y-2">
                        <a href="{{ route('dashboard') }}" class="flex items-center rounded-xl px-4 py-3 transition-colors {{ request()->routeIs('dashboard') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                        ...
                        <a href="{{ route('expenses.index') }}" class="flex items-center rounded-xl px-4 py-3 transition-colors {{ request()->routeIs('expenses.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                            <svg class="mr-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            Expenses
                        </a>

                        <a href="{{ route('suppliers.index') }}" class="flex items-center rounded-xl px-4 py-3 transition-colors {{ request()->routeIs('suppliers.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                            <svg class="mr-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                            Suppliers
                        </a>

                        <a href="{{ route('customers.index') }}" class="flex items-center rounded-xl px-4 py-3 transition-colors {{ request()->routeIs('customers.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                        ...
                        <a href="{{ route('settings.edit') }}" class="flex items-center rounded-xl px-4 py-3 transition-colors {{ request()->routeIs('settings.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                            <svg class="mr-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                            Settings
                        </a>
                    </nav>
                    ...
                    <div class="mt-3 border-t border-gray-800 pt-3">
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="flex w-full items-center rounded-xl px-4 py-3 text-red-400 transition-colors hover:bg-red-500/10">
